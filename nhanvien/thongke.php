...
FROM 
    (SELECT DISTINCT YEAR(ngaydat) AS Nam FROM hoadon) y
    CROSS JOIN Months m
    LEFT JOIN hoadon hd ON y.Nam = YEAR(hd.ngaydat) AND m.Thang = MONTH(hd.ngaydat)
GROUP BY 
    y.Nam,
    m.Thang
ORDER BY 
...
$dataPoints = array();
foreach ($data as $row) {
    $dataPoints[] = array("label" => "Tháng " . $row['Thang'], "y" => $row['TongSoTien']);
}
                    ...
                    <div class="row">
                        <div class="col-lg-3 col-md-3 col-sm-12 pr-0 mb-3">
                            <div class="card text-white bg-primary">
                                <div class="card-header"><i class="fa fa-shopping-bag"></i> Tổng doanh thu</div>
                                <div class="card-body">
                                    <?php
                                    include("../config.php");
                                    $sql = "select sum(tongtien) as tongtien  from hoadon";
                                    $loai_san_pham = mysqli_query($ket_noi, $sql);
                                    while ($row = mysqli_fetch_array($loai_san_pham)) {; ?>
                                        <h3 class="card-title"><?php echo number_format($row["tongtien"], 0, '.', ','); ?> VND</h3>
